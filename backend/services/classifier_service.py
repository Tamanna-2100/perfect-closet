...
SEASON_CENTROIDS: Dict[str, Tuple[float, float, float, float, float]] = {
    ...
    "Bright Spring": (66.0, 12.0, 18.0, 50.0, 21.6),
    ...
    "Soft Autumn": (60.0, 12.0, 17.0, 28.0, 20.8),
    ...
    "Bright Winter": (68.0, 11.0, 7.0, 65.0, 13.0),
}
...
class ClassifierService:
    ...
    def classify(self, features: ExtractedFeatures) -> SeasonResult:
        ...
        - Distance-weighted matching against seasonal centroids with continuous penalties.
        """
        skin = features.skin
        ...
        # Weighting: Lightness (0.9), undertone a (1.2), undertone b (1.5), contrast (0.9), chroma (0.7)
        weights = np.array([0.9, 1.2, 1.5, 0.9, 0.7], dtype=np.float32)

        # Determine undertone:
        # Warm undertone: positive b*, golden/peach chromaticity
        # Cool undertone: lower b*, pink/blue chromaticity
        is_warm = skin.b > 13.5 or (skin.b > 10.5 and skin.ita < 42.0)

        best_season = "Bright Spring"
        ...
        for season_name, centroid in SEASON_CENTROIDS.items():
            ...
            # Continuous undertone alignment check
            season_is_warm = "Spring" in season_name or "Autumn" in season_name
            if is_warm and not season_is_warm:
                dist += 14.0  # Warm user penalized for cool seasons
            elif not is_warm and season_is_warm:
                dist += 14.0  # Cool user penalized for warm seasons

            # Continuous/graduated contrast penalties:
            # "Soft" seasons require low contrast (< 35). Penalize if contrast > 35
            if "Soft" in season_name and contrast > 35.0:
                dist += (contrast - 35.0) * 1.1
            # "Bright" seasons require vivid contrast (> 36). Penalize if contrast < 36
            elif "Bright" in season_name and contrast < 36.0:
                dist += (36.0 - contrast) * 1.1

            # "Light" seasons require higher skin lightness (L > 68)
            if "Light" in season_name and skin.L < 65.0:
                dist += (65.0 - skin.L) * 0.8
            # "Deep" seasons require deeper skin lightness or dark features (L < 52)
            elif "Deep" in season_name and skin.L > 58.0:
                dist += (skin.L - 58.0) * 0.8
